...
fdisk $SiZe=>10 @UniT=>M $path=>"/tmp/archivos/fase 1/D1.dsk"    @type=>P @fit=>FF $nAMe=>"PRI 1"
fdisk $path=>"/tmp/archivos/fase 1/D1.dsk" $SiZe=>5120 	  @fit=>BF $nAMe=>"PRI 2"
fdisk $path=>"/tmp/archivos/fase 1/D1.dsk" @type=>E $nAMe=>"EXT" $SiZe=>51200
fdisk @type=>L $SiZe=>5120 @Unit=>K $path=>"/tmp/archivos/fase 1/D1.dsk" $nAMe=>"LOG 1"
fdisK $nAMe=>"LOG 2" $SiZe=>5120 $path=>"/tmp/archivos/fase 1/D1.dsk" @type=>L
fdisk @type=>L $path=>"/tmp/archivos/fase 1/D1.dsk" $nAMe=>"LOG 3" $SiZe=>5120 @uniT=>K
fdiSk @type=>L $SiZe=>5120 @unIt=>K $PAth=>"/tmp/archivos/fase 1/D1.dsk" $nAMe=>"LOG 4"
fdisk @type=>L $SiZe=>5120 @unit=>K $path=>"/tmp/archivos/fase 1/D1.dsk" $nAMe=>"LOG 5"
FdisK $SiZe=>5120 $nAMe=>"LOG 6" @UNit=>K $path=>"/tmp/archivos/fase 1/D1.dsk" @type=>L
fdIsk @type=>L $SiZe=>5120 @unit=>K $path=>"/tmp/archivos/fase 1/D1.dsk" $nAMe=>"LOG 7"
fdIsk @type=>L $SiZe=>5120 @unit=>K $path=>"/tmp/archivos/fase 1/D1.dsk" $nAMe=>"LOG 8" @fit=>BF
fdisk @unit=>B $path=>"/tmp/archivos/fase 1/D1.dsk" $SiZe=>5242880 $nAMe=>"PRI 3"
...
fdisk $SiZe=>10 @UniT=>M $path=>"/tmp/archivos/fase 1/D3.dsk"     @type=>P @fit=>FF $nAMe=>"PRI 1"
fdisk $path=>"/tmp/archivos/fase 1/D3.dsk" $SiZe=>5120 	 @fit=>BF $nAMe=>"PRI 2"
FdisK $path=>"/tmp/archivos/fase 1/D3.dsk" @type=>E $nAMe=>"EXT" $SiZe=>51200
fdisk @type=>L $SiZe=>5120 @Unit=>K $path=>"/tmp/archivos/fase 1/D3.dsk" $nAMe=>"LOG 1"
fdisK $nAMe=>"LOG 2" $SiZe=>5120 $path=>"/tmp/archivos/fase 1/D3.dsk" @type=>L
fdisk @type=>L $path=>"/tmp/archivos/fase 1/D3.dsk" $nAMe=>"LOG 3" $SiZe=>5120 @uniT=>K
fdiSk $SiZe=>5120 @unIt=>K $PAth=>"/tmp/archivos/fase 1/D3.dsk" $nAMe=>"LOG 4" @type=>L
fdisk @type=>L $SiZe=>5120 @unit=>K $path=>"/tmp/archivos/fase 1/D3.dsk" $nAMe=>"LOG 5"
FdisK $SiZe=>5120 $nAMe=>"LOG 6" @UNit=>K $path=>"/tmp/archivos/fase 1/D3.dsk" @type=>L
fdIsk @type=>L $SiZe=>5120 @unit=>K $path=>"/tmp/archivos/fase 1/D3.dsk" $nAMe=>"LOG 7"
fdIsk @type=>L $SiZe=>5120 @unit=>K $path=>"/tmp/archivos/fase 1/D3.dsk" $nAMe=>"LOG 8" @fit=>BF
fdisk @unit=>B $path=>"/tmp/archivos/fase 1/D3.dsk" $SiZe=>5242880 $nAMe=>"PRI 3"

/*Verificar generacion de ids al montar*/
/*Debería generar los ids: vda1, vdb1, vda2, vdb2*/
/*vda para el d3.dsk y vdb para d1.dsk*/
mouNt $nAMe=>"PRI 3"  $path=>"/tmp/archivos/fase 1/D3.dsk"
moUnt $PAth=>"/tmp/archivos/fase 1/D1.dsk" $nAMe=>"PRI 3"
mOunt $nAMe=>"PRI 2" $path=>"/tmp/archivos/fase 1/D3.dsk"
mouNt $path=>"/tmp/archivos/fase 1/D1.dsk" $nAMe=>"PRI 2"

/*Validaciones al montar, no deberia cargarlos*/
moUnt $nAMe=>PX $path=>"/tmp/archivos/fase 1/D3.dsk"
moUNt $path=>"/tmp/archivos/fase 1/DX.dsk" $nAMe=>"PRI 3"

/*Listar las particiones montadas*/
mouNt
/*SAlida es la siguiente*/
/*id=>vda1 $path=>"/tmp/archivos/fase 1/D3.dsk" $nAMe=>"PRI 3"*/
/*id=>vdb1 $PAth=>"/tmp/archivos/fase 1/D1.dsk" $nAMe=>"PRI 3"*/
/*id=>vda2 $path=>"/tmp/archivos/fase 1/D3.dsk" $nAMe=>"PRI 2"*/
/*id=>vdb2 $path=>"/tmp/archivos/fase 1/D1.dsk" $nAMe=>"PRI 2"*/
...
fdisk @delete=>Fast $nAMe=>"PRI 2" $path=>"/tmp/archivos/fase 1/D1.dsk"
fdisk $path=>"/tmp/archivos/fase 1/D1.dsk" $nAMe=>"LOG 1" @delete=>FasT
fDisk $nAMe=>"LOG 4" @delete=>FaST $PAth=>"/tmp/archivos/fase 1/D1.dsk"
fdisK $path=>"/tmp/archivos/fase 1/D1.dsk" $nAMe=>"LOG 5" @delete=>FAsT
...
fdisk @Delete=>Full $nAMe=>"PRI 1" @Delete=>Full $path=>"/tmp/archivos/fase 1/D3.dsk"
...
fdiSk @unit=>M $SiZe=>5 $path=>"/tmp/archivos/fase 1/D1.dsk" @type=>P @fit=>FF $nAMe=>"PRI 2"
fdisk $path=>"/tmp/archivos/fase 1/D1.dsk" $SiZe=>10240 @type=>L $nAMe=>"LOG 45" @unit=>K
fdisk $path=>"/tmp/archivos/fase 1/D1.dsk" $SiZe=>5120 @type=>L $nAMe=>"LOG 1" @unit=>K


/*Disco 3: Primaria2 EXTENDIDA LOGICA1 PRIMARIA1 PRIMARIA3*/
fdisk @type=>E $path=>"/tmp/archivos/fase 1/D3.dsk" $SiZe=>10240 $nAMe=>"EXT"
fdiSk $nAMe=>"LOG 1" @unit=>K $path=>"/tmp/archivos/fase 1/D3.dsk" @type=>L $SiZe=>5120
fdisk $SiZe=>10240 $nAMe=>"PRI 1" $path=>"/tmp/archivos/fase 1/D3.dsk" @type=>P
...
fdisk $path=>"/tmp/archivos/fase 1/D1.dsk" $nAMe=>"PRI 1" @add=>3
/*(porque no debe quedar espacio negativo) debe mostrar mensaje de cuanto puede quitarle*/
fdisk @add=>-20 @unit=>M $path=>"/tmp/archivos/fase 1/D1.dsk" $nAMe=>"PRI 1" @unit=>M
/*(Porque no hay suficiente espacio)*/
fdisk @unit=>M $path=>"/tmp/archivos/fase 1/D1.dsk" $nAMe=>"LOG 7" @add=>150

/*Quita y agrega 1 mb de la primer particion*/
fdisk @unit=>M $path=>"/tmp/archivos/fase 1/D1.dsk" $nAMe=>"PRI 1" @add=>-1
...
fdisk @unit=>M $path=>"/tmp/archivos/fase 1/D1.dsk" $nAMe=>"LOG 45" @add=>$1
